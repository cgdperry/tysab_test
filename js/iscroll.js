(function() {
	var m = Math, mround = function(r) {
		return r >> 0;
			...
			: (/firefox/i).test(navigator.userAgent) ? "Moz" : (/trident/i)
					.test(navigator.userAgent) ? "ms" : "opera" in window ? "O"
					: "", isAndroid = (/android/gi).test(navigator.appVersion), isIDevice = (/iphone|ipad/gi)
			.test(navigator.appVersion), isPlaybook = (/playbook/gi)
			.test(navigator.appVersion), isTouchPad = (/hp-tablet/gi)
			.test(navigator.appVersion), has3d = "WebKitCSSMatrix" in window
			&& "m11" in new WebKitCSSMatrix(), hasTouch = "ontouchstart" in window
			&& !isTouchPad, hasTransform = vendor + "Transform" in document.documentElement.style, hasTransitionEnd = isIDevice
			|| isPlaybook, nextFrame = (function() {
		...
		return window.cancelRequestAnimationFrame
	...
	})(), RESIZE_EV = "onorientationchange" in window ? "orientationchange"
			...
			: "mouseup", WHEEL_EV = vendor == "Moz" ? "DOMMouseScroll"
			: "mousewheel", trnOpen = "translate" + (has3d ? "3d(" : "("), trnClose = has3d ? ",0)"
		...
		that.scroller.style[vendor + "TransitionProperty"] = that.options.useTransform ? "-"
		...
		if (that.options.useTransform) {
			that.scroller.style[vendor + "Transform"] = trnOpen + that.x
					+ "px," + that.y + "px" + trnClose;
		} else {
	...
	iScroll.prototype = {
		...
		_checkDOMChanges : function() {
			if (this.moved
					...
					|| (this.scrollerW == this.scroller.offsetWidth
							* this.scale && this.scrollerH == this.scroller.offsetHeight
							* this.scale)) {
				return;
			}
			this.refresh();
		...
		_scrollbar : function(dir) {
			...
			if (!that[dir + "Scrollbar"]) {
				if (that[dir + "ScrollbarWrapper"]) {
					if (hasTransform) {
						that[dir + "ScrollbarIndicator"].style[vendor
								+ "Transform"] = "";
					}
			...
			if (!that[dir + "ScrollbarWrapper"]) {
				...
				bar.style.cssText += ";pointer-events:none;-" + vendor
				...
				bar.style.cssText += ";pointer-events:none;-"
						...
						+ vendor + "-transition-duration:0;-" + vendor
			...
			if (dir == "h") {
				...
				that.hScrollbarIndicatorSize = m.max(mround(that.hScrollbarSize
				...
				that.vScrollbarIndicatorSize = m.max(mround(that.vScrollbarSize
		...
		_pos : function(x, y) {
			...
			if (this.options.useTransform) {
				this.scroller.style[vendor + "Transform"] = trnOpen + x + "px,"
						+ y + "px" + trnClose + " scale(" + this.scale + ")";
			} else {
				x = mround(x);
				y = mround(y);
            ...
            if (this.options.onScrollMove) {
				this.options.onScrollMove.call(this, x, y);
			}
		},
		_scrollbarPos : function(dir, hidden) {
			...
			if (pos < 0) {
				...
				if (pos > that[dir + "ScrollbarMaxScroll"]) {
					if (!that.options.fixedScrollbar) {
						size = that[dir + "ScrollbarIndicatorSize"]
								- mround((pos - that[dir + "ScrollbarMaxScroll"]) * 3);
						if (size < 8) {
							size = 8;
						}
						that[dir + "ScrollbarIndicator"].style[dir == "h" ? "width"
			...
			that[dir + "ScrollbarWrapper"].style.opacity = hidden
					&& that.options.hideScrollbar ? "0" : "1";
			that[dir + "ScrollbarIndicator"].style[vendor + "Transform"] = trnOpen
		...
		_start : function(e) {
			...
			if (that.options.momentum) {
				if (that.options.useTransform) {
					matrix = getComputedStyle(that.scroller, null)[vendor
		...
		_move : function(e) {
			...
			if (that.options.zoom && hasTouch && e.touches.length > 1) {
				...
				if (scale < that.options.zoomMin) {
					scale = 0.5 * that.options.zoomMin
				...
				newX = this.originX - this.originX * that.lastScale + this.x,
						newY = this.originY - this.originY * that.lastScale
								+ this.y;
				this.scroller.style[vendor + "Transform"] = trnOpen + newX
						+ "px," + newY + "px" + trnClose + " scale(" + scale
						+ ")";
			...
			} moved to _pos to fire more frequently*/
		},
		_end : function(e) {
		...
		_momentum : function(dist, time, maxDistUpper, maxDistLower, size) {
			var deceleration = 0.0006, speed = m.abs(dist) / time, newDist = (speed * speed)
		...
		zoom : function(x, y, scale, time) {
			...
			that.scroller.style[vendor + "Transform"] = trnOpen + that.x
					+ "px," + that.y + "px" + trnClose + " scale(" + scale
					+ ")";
			that.zoomed = false;
		},
		isReady : function() {
